...
class Product(db.Model):
    ...
    # Exchange preferences - what seller wants in return
    exchange_preference = db.Column(db.Text, nullable=True)  # Apa yang diinginkan penjual untuk tukar
...
class TransactionReceipt(db.Model):
    ...
    proposal_id = db.Column(db.Integer, db.ForeignKey('barter_proposal.id'), nullable=True)  # Allow null for chat deals

    # Transaction details - PERMANENT STORAGE
    ...
    # Product details for permanent record
    offered_product_title = db.Column(db.String(200), nullable=True)
    offered_product_condition = db.Column(db.String(50), nullable=True)
    wanted_product_title = db.Column(db.String(200), nullable=True)
    wanted_product_condition = db.Column(db.String(50), nullable=True)

    # Exchange terms
    exchange_terms = db.Column(db.Text, nullable=True)  # What seller wanted in exchange
    payment_method = db.Column(db.String(50), nullable=True)
    currency_type = db.Column(db.String(10), default='IDR')
    ...
    is_permanent = db.Column(db.Boolean, default=True)  # Ensure permanent storage
    archived_at = db.Column(db.DateTime, nullable=True)  # For archival purposes
    created_at = db.Column(db.DateTime, default=datetime.utcnow)
    ...
    def get_receipt_data_for_print(self):
        """Get all receipt data formatted for printing/permanent storage"""
        return {
            'receipt_number': self.receipt_number,
            'transaction_date': self.created_at.strftime('%d %B %Y, %H:%M WIB'),
            'seller': {
            ...
            'items': {
                'offered': {
                    'title': self.main_item_offered,
                    'condition': self.offered_product_condition
                },
                'wanted': {
                    'title': self.main_item_wanted,
                    'condition': self.wanted_product_condition
                }
            },
            'financial': {
                'agreed_price': self.final_agreed_price,
...
class ChatMessage(db.Model):
    ...
    def get_deal_status(self):
        """Get human-readable deal status"""
        if self.message_type != 'deal' and self.message_type != 'offer':
            return None

        if self.is_expired():
            return 'Expired'
        elif self.deal_accepted is True:
            return 'Diterima'
        elif self.deal_accepted is False:
            return 'Ditolak'
        else:
...
class TopupItem(db.Model):
    """Model untuk menyimpan detail barang yang akan diberikan pembeli saat topup"""
    id = db.Column(db.Integer, primary_key=True)
    receipt_id = db.Column(db.Integer, db.ForeignKey('transaction_receipt.id'), nullable=False)
    buyer_id = db.Column(db.Integer, db.ForeignKey('user.id'), nullable=False)

    # Informasi detail barang yang akan diberikan
    item_title = db.Column(db.String(200), nullable=False)
    item_description = db.Column(db.Text, nullable=False)
    item_condition = db.Column(db.String(20), nullable=False)  # New, Like New, Good, Fair
    estimated_value = db.Column(db.Float, nullable=False)
    brand = db.Column(db.String(100), nullable=True)
    model = db.Column(db.String(100), nullable=True)
    year_purchased = db.Column(db.Integer, nullable=True)
    original_price = db.Column(db.Float, nullable=True)

    # Proof of ownership & authenticity
    purchase_proof = db.Column(db.String(500), nullable=True)  # Receipt/invoice URL/file
    authenticity_proof = db.Column(db.String(500), nullable=True)  # Warranty card, serial number
    additional_accessories = db.Column(db.Text, nullable=True)  # Charger, box, etc.

    # Physical condition details
    physical_condition_notes = db.Column(db.Text, nullable=True)
    functional_condition_notes = db.Column(db.Text, nullable=True)
    defects_or_issues = db.Column(db.Text, nullable=True)

    # Status tracking
    status = db.Column(db.String(20), default='pending')  # pending, approved, rejected, delivered
    seller_approval = db.Column(db.Boolean, nullable=True)  # True = approved, False = rejected
    seller_feedback = db.Column(db.Text, nullable=True)

    # Delivery commitment
    delivery_method = db.Column(db.String(50), nullable=True)  # pickup, shipping, courier
    ...
    # Relationships
    receipt = db.relationship('TransactionReceipt', backref='topup_items')
    buyer = db.relationship('User', backref='topup_items')

    def __repr__(self):
        return f'<TopupItem {self.item_title}>'

class TopupItemImage(db.Model):
    """Model untuk menyimpan foto-foto barang topup"""
    id = db.Column(db.Integer, primary_key=True)
    topup_item_id = db.Column(db.Integer, db.ForeignKey('topup_item.id'), nullable=False)
    image_filename = db.Column(db.String(120), nullable=True)
    image_url = db.Column(db.String(500), nullable=True)
    image_type = db.Column(db.String(50), nullable=False)  # main, detail, condition, proof, accessory
    description = db.Column(db.String(200), nullable=True)  # Description of what the image shows
    is_primary = db.Column(db.Boolean, default=False)
    created_at = db.Column(db.DateTime, default=datetime.utcnow)

    # Relationships
    topup_item = db.relationship('TopupItem', backref='images')
    ...
    def __repr__(self):
        return f'<TopupItemImage {self.id} for TopupItem {self.topup_item_id}>'