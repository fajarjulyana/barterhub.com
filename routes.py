...
from app import app, db, login_manager # Assuming login manager is initialized here
from models import User, Product, Category, Message, BarterProposal, Negotiation, TransactionReceipt, ShippingDetail, ChatMessage, ChatConversation # Added missing imports
import uuid
from datetime import datetime # Moved datetime import to the top
...
# Chat Management Routes (Legacy - use real-time chat instead)
@app.route('/send_chat_message_legacy', methods=['POST'])
@login_required
def send_chat_message_legacy():
...
def get_recent_chats():
    """Get recent chat conversations for current user"""
    try:
        ...
        conversations = ChatConversation.query.filter(
            (ChatConversation.seller_id == current_user.id) | 
            (ChatConversation.buyer_id == current_user.id)
        ).order_by(ChatConversation.last_message_at.desc()).limit(5).all()

        chat_data = []
        for conv in conversations:
            # Determine other user
            if conv.seller_id == current_user.id:
                other_user = User.query.get(conv.buyer_id)
                user_role = 'seller'
            else:
                other_user = User.query.get(conv.seller_id)
                user_role = 'buyer'
            ...
            chat_data.append({
                ...
                'last_message_at': conv.last_message_at.strftime('%H:%M') if conv.last_message_at else None,
        ...
        logging.error(f"Error fetching recent chats: {e}") # Log the error
...
@app.route('/chat_with_user/<int:user_id>')
@app.route('/chat_with_user/<int:user_id>/<int:product_id>')
@login_required
def chat_with_user(user_id, product_id=None):
...
@app.route('/api/get_sellers')
@login_required
def get_sellers():
    """Get list of active sellers"""
    try:
        # Get sellers who have active products
        sellers = User.query.filter(
            User.user_role == 'seller',
            User.active == True,
            User.is_suspended == False,
            User.id != current_user.id
        ).join(Product).filter(Product.is_available == True).distinct().limit(20).all()
        ...
        for seller in sellers:
            seller_list.append({
                ...
                'active_products': Product.query.filter_by(owner_id=seller.id, is_available=True).count()
            })
